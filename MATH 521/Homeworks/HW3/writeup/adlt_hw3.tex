...
\begin{scratch}
For (a), use the defining property of $b$ with $a=1$ (the multiplicative identity given by the
field axioms). Then compare $b\cdot 1$ computed in two ways: (i) by the hypothesis on $b$ and
(ii) by the fact that $1$ is a multiplicative identity in $F$.
\end{scratch}
...
\]
Let $1\in F$ denote the multiplicative identity provided by the field axioms. Plugging in $a=1$
gives
\[
b\cdot 1 = 1.
\]
On the other hand, since $1$ is a multiplicative identity, we also have $b\cdot 1 = b$.
Therefore $b=1$, proving that the multiplicative identity is unique.
