...
\begin{scratch}
The hypotheses say that $g$ is simultaneously a \emph{left inverse} and a \emph{right inverse}
for $f$:
\[
...
\]
A left inverse forces injectivity (apply $g$ to $f(a_1)=f(a_2)$), and a right inverse forces
surjectivity (every $b\in B$ equals $f(g(b))$).
\end{scratch}
...
g(f(a_1))=g(f(a_2)).
\]
Using $g\circ f=\mathrm{id}_A$, this becomes $a_1=a_2$. Hence $f$ is injective.
