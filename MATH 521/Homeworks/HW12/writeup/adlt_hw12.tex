...
For the reverse direction, I assume the closure condition holds for all $A$. To prove continuity, I take an arbitrary convergent sequence $x_n\to x$ and consider the set $A=\{x_n:n\in\mathbb{N}\}$. Then $x\in \overline{A}$, so $f(x)\in \overline{f(A)}$. That means there is a sequence in $f(A)$ converging to $f(x)$, and I use that to conclude $f(x_n)\to f(x)$.

So the entire proof hinges on the relationship between closures and sequences, and how continuity preserves limits.
\end{scratch}
...
Let $\{x_n\}$ be a sequence in $X$ such that $x_n \to x$. Define
\[
A := \{x_n : n\in \mathbb{N}\}.
\]
Then $x\in \overline{A}$. By hypothesis,
\[
f(x)\in \overline{f(A)}.
\]
So there exists a sequence $\{f(x_{n_k})\}\subseteq f(A)$ such that
\[
f(x_{n_k}) \to f(x).
\]

Now we show that $f(x_n)\to f(x)$. Let $\varepsilon>0$. Since $f(x_{n_k})\to f(x)$, there exists $K$ such that
\[
d_Y(f(x_{n_k}),f(x))<\varepsilon \quad \text{for all } k\ge K.
\]
Since $x_n\to x$, the sequence $\{x_n\}$ is eventually close to $x$, and thus the subsequence $\{x_{n_k}\}$ eventually captures the limiting behavior. Hence for all sufficiently large $n$, we also have
\[
...
\]
By the Intermediate Value Theorem, there exists some $t\in(0,1)$ such that
\[
\varphi(t)=0.
\]
